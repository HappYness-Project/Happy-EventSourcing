...
namespace HP.Controllers
{
    ...
    public class PersonsController : ControllerBase
    {
        ...
        [HttpGet]
        public async Task<IEnumerable<PersonDetailsDto>> Get() => await _mediator.Send(new GetPersonList());
        ...
        public async Task<IActionResult> Create([FromBody]CreatePersonRequest request, CancellationToken token = default)
        {
            ...
            return Ok(await _mediator.Send(new CreatePersonCommand(request.PersonName, request.Email, request.PersonType, request.GroupId)));
        }
        [HttpPut("{personId}")]
        public async Task<CommandResult> Update(Guid personId, [FromBody]UpdatePersonRequest request) =>
            await _mediator.Send(new UpdatePersonCommand(personId, request.PersonType, request.GoalType, request.GroupId));
